...
for(i in names(ascetic_results)) {
    ...
    if(nrow(arcs)>0) {
        ...
        for(j in 1:nrow(evo_steps)) {
            ...
            curr_co = sort(unique(names(which(rowSums(ascetic_results[[cancer_type]][["dataset"]][,c(curr_p,curr_c)])==2))))
            CO_OCCURRENCE[j] = length(curr_co)
            valid_models = sort(unique(names(which(rowSums(ascetic_results[[cancer_type]][["dataset"]][,c(curr_p,curr_c)])==2))))
...
EvoSteps = all_arcs[[1]]
EvoSteps = EvoSteps[which(!is.na(EvoSteps$EVO_SCORE)),]
rownames(EvoSteps) = 1:nrow(EvoSteps)

# save results
save(EvoSteps,file="results/EvoSteps.RData")
write.xlsx(EvoSteps,file="results/EvoSteps.xlsx",sheetName="Myelodysplastic Syndromes",rowNames=FALSE,colNames=TRUE)
